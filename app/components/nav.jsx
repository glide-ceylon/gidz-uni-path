...
export default function Nav() {
  ...
  return (
    <header className="bg-gradient-to-r from-purple-0 to-teal-50 w-full bg-[length:400%_400%] animate-gradient-move shadow-sm">
      <div className="container mx-auto flex justify-between lg:w-10/12 items-center p-4">
        ...
        <nav className="hidden md:flex items-center relative">
          <Link
            href="/"
            className="ml-2 text-gray-600 hover:text-gray-900 active:font-bold px-4 py-2 rounded-md transition duration-200"
          >
            Home
          </Link>
          ...
          >
            <Link href="/apply-now">
              <button
                className="ml-2 text-gray-600 hover:text-gray-900 active:font-bold px-4 py-2 rounded-md flex items-center transition duration-200"
                aria-haspopup="true"
            ...
            {isVisaDropdownOpen && (
              <div className="absolute top-full left-0 w-48 bg-white border rounded-md shadow-lg z-50">
                <Link
                  href="/apply-now/student"
                  className="block px-4 py-2 text-gray-600 hover:text-gray-900 hover:bg-gray-100 transition duration-200"
                >
                  Student Visa
                </Link>
                <Link
        ...
        {!isLoggedIn ? (
          <Link
            href="/login"
            className="ml-6 bg-teal-500 text-white active:font-bold px-4 py-2 rounded-md shadow-md transition duration-200 hover:bg-teal-600"
          >
            Login
          </Link>
        ) : (
        ...
        <div className="fixed inset-0 z-50 bg-white bg-opacity-95 transition-opacity duration-300 ease-in-out">
          ...
          <nav className="flex flex-col items-center justify-center h-full space-y-6">
            <Link
              ...
              className="text-2xl font-medium text-gray-700 hover:text-teal-600 transition duration-200"
            >
              Home
            </Link>
            ...
            <div className="w-full flex flex-col items-center">
              <button
                ...
                className="flex items-center justify-center w-full text-2xl font-medium text-gray-700 hover:text-teal-600 transition duration-200"
              >
                Visa
                <svg
              ...
              {isMobileVisaOpen && (
                <div className="mt-4 flex flex-col items-center space-y-4">
                  <Link
                    href="/apply-now/student"
                    onClick={() => setIsMobileMenuOpen(false)}
                    className="text-xl font-medium text-gray-600 hover:text-teal-500 transition duration-200"
                  >
                    Student Visa
                  </Link>
                  <Link
            ...
            {!isLoggedIn ? (
              <Link
                href="/login"
                onClick={() => setIsMobileMenuOpen(false)}
                className="bg-teal-500 text-white px-6 py-3 rounded-md text-2xl font-medium shadow-lg hover:bg-teal-600 transition duration-200"
              >
                Login
              </Link>
            ) : (
